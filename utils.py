...
def acc(mat, lag):
    ...
    if lag > n:
        raise ValueError('lag must be smaller than the amino acids in the sequence')
    ...
    for j in range(p):
        for i in range(lag):
            acc1[j, i] = np.sum(mat[j, 0:n - i] * mat[j, i:n]) / (n - i)

    # auto cross covariance: p^2 - p elements

    acc2 = np.zeros((p**2 - p, lag))

    idx = list(combinations(range(p), 2)) + [(y, x) for (x, y) in combinations(range(p), 2)]

    for j in range(len(idx)):
        for i in range(lag):
            acc2[j, i] = np.sum(mat[idx[j][0], 0:n - i] * mat[idx[j][1], i:n]) / (n - i)
